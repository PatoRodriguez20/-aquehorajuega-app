...
export default async function Home() {
  ...
  const proximosPartidos = partidos.filter(
    (p) => p.fecha && new Date(p.fecha) > ahora,
  );
    ...
    <>
      <div
        style={{
          ...
          zIndex: 0,
          background:
            "radial-gradient(ellipse 80% 50% at 50% -10%, rgba(35,188,221,0.12) 0%, transparent 70%)",
        }}
      />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          >
            <span style={{ color: "#6B8BA4" }}>
              {formatFechaHero(proximoPartido.fecha)}
            </span>
            <span
              style={{
                fontSize: "0.78rem",
                ...
                border: `1px solid ${
                  esLocal ? "rgba(46,204,113,0.35)" : "rgba(255,140,0,0.4)"
                }`,
                color: esLocal ? "#2ecc71" : "#ff8c00",
              }}
            >
